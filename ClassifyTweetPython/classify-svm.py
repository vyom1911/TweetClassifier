...
#SVM_cv
text_clf_svm_cv = Pipeline([('vect', CountVectorizer()),
                         ('clf-svm', SGDClassifier(loss='hinge',penalty='l2',alpha=1e-3,n_iter=5, random_state=42))])
_ = text_clf_svm_cv.fit(x_train, y_train)
pred1 = text_clf_svm_cv.predict(x_test)

#saving
with open('model_svm_small_cv.pkl','wb')as fout:
    pickle.dump((text_clf_svm_cv), fout)



#SVM_tfidf
text_clf_svm_tfidf = Pipeline([('tfidfV', TfidfVectorizer(min_df=1,stop_words='english')),
                         ('clf-svm', SGDClassifier(loss='hinge',penalty='l2',alpha=1e-3,n_iter=5, random_state=42))])

_ = text_clf_svm_tfidf.fit(x_train, y_train)
pred2 = text_clf_svm_tfidf.predict(x_test)
#saving
with open('model_svm_small_tfidf.pkl','wb')as fout:
    pickle.dump((text_clf_svm_tfidf), fout)




#Classifying using SVM_both
text_clf_svm_both = Pipeline([('vect', CountVectorizer()),
                         ('tfidf', TfidfTransformer()),
                         ('clf-svm', SGDClassifier(loss='hinge',penalty='l2',alpha=1e-3,n_iter=5, random_state=42))])

_ = text_clf_svm_both.fit(x_train, y_train)
pred3 = text_clf_svm_both.predict(x_test)

with open('model_svm_small_both.pkl','wb')as fout:
    pickle.dump((text_clf_svm_both), fout)
...
#saving svm
#with open('modelSvm.pkl', 'wb') as fout:
#    pickle.dump(text_clf_nb, fout)
...
print("accuracy for svm-cv = ", accuracy*100,"%")
...
print("accuracy for svm-tfidf = ", accuracy*100,"%")

count=0
for i in range (len(pred3)):
    if pred3[i]==actual_prediction[i]:
        count=count+1
        
accuracy = count/len(pred3)

print("accuracy for svm-both = ", accuracy*100,"%")
